...
(function() {
  jQuery(function() {
    var hash, loading_products, page_regexp, pushPage;
    page_regexp = /\d+$/;
    ...
    if ($('#infinite-scrolling').size() > 0) {
      ...
      $(window).bindWithDelay('scroll', function() {
        ...
        if (more_products_url && $(window).scrollTop() > $(document).height() - $(window).height() - 60) {
          ...
          $.getScript(more_products_url, function() {
            return pushPage(more_products_url.match(page_regexp)[0]);
          });
        }
      }, 100);
    }
    if ($('#with-button').size() > 0) {
      ...
      $('#load_more_products').show().click(function() {
        var $this, more_products_url;
        if (!loading_products) {
          loading_products = true;
          more_products_url = $('#with-button .next_page a').attr('href');
          if (more_products_url) {
            $this = $(this);
            $this.html('<img src="/assets/ajax-loader.gif" alt="Loading..." title="Loading..." />').addClass('disabled');
            $.getScript(more_products_url, function() {
              if ($this) {
                $this.text('More products').removeClass('disabled');
              }
              pushPage(more_products_url.match(page_regexp)[0]);
              return loading_products = false;
            });
          }
        }
      });
